...
```
2. Open the terminal and make sure your terminal's working directory is the `webcrawler` folder which has the `pom.xml` file in it. First click on the pom.xml and then navigate to header tab View -> Tools Window -> Terminal (or on a MacOS, use the shortcut option + F12). Once you have a terminal open, package the compiled code into a distributable format, such as a JAR, by typing (or copy-pasting) the following `mvn package` command into the terminal and pressing the `Enter` key:
```
mvn package
```
3. Next, after a BUILD SUCCESS, run the performance profiler with the following commands:
```
...
```
The terminal output will display the crawl results in a JSON format of the top 5 words and how many wordCounts.
```
{
"wordCounts" : {
"video" : 651,
...
"urlsVisited" : 55
```
4. Open the [profileData.txt file](starter/webcrawler/profileData.txt) to see the performance comparison of the single-threaded vs multi-threaded webcrawler performance:

```java
Run at Thu, 24 Apr 2025 21:09:03 GMT
com.udacity.webcrawler.ParallelWebCrawler#crawl took 0m 5s 207ms
